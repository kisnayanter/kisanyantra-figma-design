        ...
        <div className="w-full h-full rounded-[28px] overflow-hidden relative flex flex-col" style={{ background: '#F8F5F0' }}>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="relative">
                ...
                <button
                  className="w-[42px] h-[42px] rounded-[14px] flex items-center justify-center text-[20px]"
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-5 -mt-8">
            ...
            >
              ...
              <button className="flex-shrink-0 text-white text-base opacity-70 hover:opacity-100">
                →
              </button>
            </div>
          ...
          {/* Bottom Nav */}
          <div 
            className="px-5 py-3 flex items-center justify-around"
            style={{ background: 'white', borderTop: '1px solid #E8E3DA' }}
          >
            {[
              { emoji: '🏠', label: 'Home', active: true },
              { emoji: '🔍', label: 'Search', badge: null },
              { emoji: '📋', label: 'Bookings', badge: 2 },
              { emoji: '👤', label: 'Profile', badge: null }
            ].map((nav) => (
              <div key={nav.label} className="relative flex flex-col items-center gap-0.5">
                <div 
                  className="text-xl relative"
                  style={{ opacity: nav.active ? 1 : 0.4 }}
                >
                  {nav.emoji}
                  {nav.badge && (
                    <div
                      className="absolute -top-1 -right-1 w-[14px] h-[14px] rounded-full flex items-center justify-center text-[8px] font-bold text-white"
                      style={{ 
                        background: '#E74C3C',
                ...
                <div 
                  ...
                  style={{ 
                    color: nav.active ? 'var(--saffron)' : 'var(--text-soft)'
                  }}
                >
                  {nav.label}
                </div>
